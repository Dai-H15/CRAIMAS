...
def get_interview(value, sets):
    if sets[value] is not None:
        return mark_safe(f"<button class='btn btn-outline-info' onclick='Main.default.open_as_window.open_as_window(\"{reverse('view_interview', args=[str(sets[value])])}\", \"interview\", 500, 800)'> 開く</button>")
    else:
